...
var _require2 = require('../models/profileSchema.js'),
  db = _require2.db;
var nodeHtmlToImage = require('node-html-to-image');
var _require3 = require('canvas'),
  createCanvas = _require3.createCanvas,
  loadImage = _require3.loadImage,
  setTransform = _require3.setTransform;
...
function updatePVMLeaderboard(_x, _x2) {
                        ...
                        var _ref2 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(buffer) {
                          ...
                          return _regeneratorRuntime().wrap(function _callee$(_context) {
                            while (1) switch (_context.prev = _context.next) {
                              ...
                              case 2:
                                img = _context.sent;
                                canvas = createCanvas(img.width * 2, img.height * 2); // double the size of the canvas
                                ctx = canvas.getContext('2d');
                                ctx.scale(2, 2);
                                scale = Math.max(ctx.canvas.width / img.width, ctx.canvas.height / img.height);
                                x = (ctx.canvas.width - img.width * scale) / 2;
                                y = (ctx.canvas.height - img.height * scale) / 2;
                                ...
                                attachment = new MessageAttachment(canvas.toBuffer(), 'leaderboard2.png'); // Send the embed with the leaderboard image to the Discord channel
                                _context.next = 14;
...
module.exports = {
  updatePVMLeaderboard: updatePVMLeaderboard
};
